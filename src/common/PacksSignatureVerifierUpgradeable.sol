...
contract PacksSignatureVerifierUpgradeable is IPacksSignatureVerifier, EIP712Upgradeable {
    ...
    function verify(CommitData memory commit, bytes memory signature) public view returns (address) {
    ...
    function _verify(CommitData memory commit, bytes memory signature) internal view returns (address) {
    ...
    /// @param signature Signature to verify
    /// @return Address of the signer
    function _verifyDigest(bytes32 digest, bytes memory signature) internal view returns (address) {
    ...
    /// @param signature Signature to verify
    /// @return Address of the signer
    function _verifyPackHash(bytes32 packHash, bytes memory signature) internal view returns (address) {
